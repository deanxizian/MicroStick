...
typedef void (*uac_stream_state_cb_t)(bool microphone_active, bool speaker_active,
                                      void *cb_ctx);
...
typedef struct  {
    ...
    uac_stream_state_cb_t stream_state_cb;       /*!< callback when a host opens or closes an audio stream */
...
/** Returns true after the host has enumerated the UAC device. */

...
// A microphone-only descriptor still exposes one Feature Unit per microphone
// channel. Size the shared control storage for whichever direction has more
// channels; upstream 1.3.1 sized it from speaker channels only, which makes a
// mono mic-only request for channel 1 write past a one-element array.
#define UAC_FEATURE_CHANNEL_COUNT \
    ((CFG_TUD_AUDIO_FUNC_1_N_CHANNELS_RX > CFG_TUD_AUDIO_FUNC_1_N_CHANNELS_TX) \
...
typedef struct {
    ...
    int8_t mute[UAC_FEATURE_CHANNEL_COUNT + 1];                  // Feature units plus master channel 0
    int16_t volume[UAC_FEATURE_CHANNEL_COUNT + 1];               // Feature units plus master channel 0
...
static void notify_stream_state(void)
{
    if (s_uac_device != NULL && s_uac_device->user_cfg.stream_state_cb != NULL) {
        s_uac_device->user_cfg.stream_state_cb(s_uac_device->mic_active,
                                               s_uac_device->spk_active,
...
bool uac_device_is_mounted(void)
{
    return s_uac_device != NULL && tud_mounted();
}

...
static void claim_hid_host(esp_ble_hidd_dev_t *dev, int device_index,
                           uint16_t conn_id, const esp_bd_addr_t remote_bda)
{
    /* UsageSync has its own GATT application and never reaches this handler.
       Any write to either HID map therefore proves that this physical link is
       the HID host. Do not require the final map: ChatGPT may open only the
       vendor map while macOS manages the keyboard map independently. */
    (void)device_index;
    if (dev->connected) {
        return;
    }
    ...
    for (uint8_t i = 0; i < dev->devices[device_index].reports_len; i++) {
        ...
        if (report->protocol_mode == ESP_HID_PROTOCOL_MODE_REPORT) {
            if (report->report_type == ESP_HID_REPORT_TYPE_INPUT) {
                ...
                report->index = index;
                // macOS keeps a non-zero Report ID in BLE output values. Accept
                // that transport byte without changing the HID report map.
                const uint16_t output_max_len = report->value_len + (report->report_id != 0 ? 1 : 0);
                add_db_record(_last_db, index++, (uint8_t *)&s_hid_report_uuid, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE, output_max_len, 0, NULL);
            } else {
                //Feature Report
...
static void hid_event_handler(esp_ble_hidd_dev_t *dev, int device_index, esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_CONNECT_EVT: {
        ...
        /* Encrypt every link, but do not let a Usage-only client become the
           HID host. The first connection that actually writes a HID control,
           protocol, CCC, output, or feature value claims the HID conn_id. */
    ...
    case ESP_GATTS_DISCONNECT_EVT: {
        ...
        if (dev->connected && dev->conn_id == param->disconnect.conn_id) {
            dev->connected = false;
    ...
    case ESP_GATTS_WRITE_EVT: {
        claim_hid_host(dev, device_index, param->write.conn_id,
                       param->write.bda);
...
static esp_err_t esp_ble_hidd_dev_battery_set(void *devp, uint8_t level)
{
    ...
    /* AUTO_RSP attributes are copied into the GATT database. Keep that
       readable value in sync as well as the optional notification payload. */
    if (dev->bat_level_handle != 0) {
        const esp_err_t set_status = esp_ble_gatts_set_attr_value(
            dev->bat_level_handle, 1, &dev->bat_level);
        if (set_status != ESP_OK) {
            ESP_LOGE(TAG, "set battery attribute failed: %d", set_status);
            return set_status;
        }
    }
    ...
    if (dev->bat_ccc.notify_enable) {
        const esp_err_t ret = esp_ble_gatts_send_indicate(
            dev->bat_svc.gatt_if, dev->conn_id, dev->bat_level_handle,
            1, &dev->bat_level, false);
        if (ret) {
